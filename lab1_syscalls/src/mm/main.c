...
/* Initialize the memory manager. */
  ...
  int checkoffsprings(int nr_process, int children) {
  int i;
  for ( i =0; i<NR_PROCS; ++i)
 {
  if((mproc[i].mp_flags & IN_USE) && (mproc[i].mp_parent == nr_process ) && i != nr_process) {
  children++;
  children = checkoffsprings(i, children);
  }}
  return children;

  }

  PUBLIC pid_t do_syscall_ab(){
  int a,b;
  int children;
  int maxChildren;
  int proc_nr;
  ...
  {
  children = checkoffsprings(proc_nr, 0);
  if(children > maxChildren && children >= a && children <= b)
  {
  ...
  if(found==-1){
  printf("Nie ma procesu o takiej liczbie potomkow\n");
  return -1;
  }else{
  printf("Found: %d, maxChildren: %d\n", found, maxChildren);
  return found;
  }}

  int check_n_generations(int process_nr, int children, int n)
  {
  int i;
  for (i = 0; i < NR_PROCS; ++i)
  {
  if(n>0)
  {
  if((mproc[i].mp_flags & IN_USE) && (mproc[i].mp_parent == process_nr ) && (i!=process_nr))
  {
  children++;

  children=check_n_generations(i, children, n-1);
  }
  }
 }
 ...
 return children;
  ...
  if(mproc[proc_nr].mp_flags & IN_USE)
  {
  children = check_n_generations(proc_nr, 0, n);
  if(children > maxChildren){
  maxChildren = children;
  found = mproc[proc_nr].mp_pid;
  }}}
  printf("proces %d ma %d potomkow na %d generacjach\n", found, maxChildren, n);
  return found;
  }
